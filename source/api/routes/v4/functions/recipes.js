...
const selectRecipeIngredients = async (connection, recipeID) => connection.query('SELECT ingredient_id AS ingredientID, quantity, unit FROM v4_recipe_ingredients WHERE recipe_id=?', recipeID);

const insertRecipeIngredient = async (connection, recipeID, ingredientID, quantity, unit) => connection.query('INSERT IGNORE INTO v4_recipe_ingredients (recipe_id, ingredient_id, quantity, unit) VALUES (?, ?, ?, ?)', [recipeID, ingredientID, quantity, unit]);

const selectRecipeFavorites = async (connection, recipeID, fridgeID) => connection.query('SELECT user_id AS userID, recipe_id AS recipeID, favorited_ts AS favoritedTS FROM v4_recipe_favorites WHERE recipe_id=? AND user_id IN (SELECT DISTINCT user_id FROM v4_fridges WHERE fridge_id=?) ORDER BY user_id ASC', [recipeID, fridgeID]);

const insertRecipeFavorite = async (connection, userID, recipeID) => connection.query('INSERT IGNORE INTO v4_recipe_favorites (user_id, recipe_id) VALUES (?, ?)', [userID, recipeID]);

const deleteRecipeFavorite = async (connection, userID, recipeID) => connection.query('DELETE FROM v4_recipe_favorites WHERE user_id=? AND recipe_id=?', [userID, recipeID]);

const insertRecipe = async (connection, recipeID, title, image, servings, cookingTime, instructions, ingredients) => connection.query('INSERT IGNORE INTO v4_recipes (recipe_id, title, image, servings, cooking_time, instructions) VALUES (?, ?, ?, ?, ?, ?)', [recipeID, title, image, servings, cookingTime, instructions])
  .then(async (results) => {
