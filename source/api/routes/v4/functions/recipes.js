...
const selectRecipes = (connection, recipeIDs, page, limit, sort, descending) => {
...
const selectFavoritedRecipes = (connection, userID, page, limit, sort, descending) => {
...
const selectRecipeIngredients = (connection, recipeID) => {
...
const insertRecipeIngredient = (connection, recipeID, ingredientID, quantity, unit) => {
...
const selectRecipeFavorites = (connection, recipeID, fridgeID) => {
...
const insertRecipeFavorite = (connection, userID, recipeID) => {
  return connection.query('INSERT IGNORE INTO v4_recipe_favorites (user_id, recipe_id) VALUES (?, ?)', [userID, recipeID]);
};

const deleteRecipeFavorite = (connection, userID, recipeID) => {
  return connection.query('DELETE FROM v4_recipe_favorites WHERE user_id=? AND recipe_id=?', [userID, recipeID]);
};

const insertRecipe = (connection, recipeID, title, image, servings, cookingTime, instructions, ingredients) => {
...
const importRecipes = (connection, recipeIDs) => {
  if (recipeIDs.length > 0) {
    ...
    connection.query(sql, [...recipeIDs])
      .then((rows) => {
        if (rows.length > 0) {
        ...
        if (recipeIDs.length > 0) {
          fetch('https://api.spoonacular.com/recipes/informationBulk?ids=' + recipeIDs.join(',') + '&includeNutrition=true&apiKey=bd1784451bab4f47ac234225bd2549ee', {
            method: 'get',
            headers: {
            ...
            .then(async (data) => {
              if (data.length > 0) {
                await Promise.all(data.forEach(async (item) => {
                  await insertRecipe(
                    connection,
                    item.id,
                    item.title,
